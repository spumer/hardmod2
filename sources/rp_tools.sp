...
public OnPluginStart()
{
	...
	RegAdminCmd("db_removeclass", Command_RemoveClass, ADMFLAG_SLAY, "");
...
public Action:Command_dublicate(Client,args)
{
	...
	SetEntityRenderColor(Ent, GetRandomInt(0, 255), GetRandomInt(0, 255), GetRandomInt(0, 255), 255);
...
public Action:Command_RemoveClass(Client,args)
{
	decl String:name[128];
	decl Ent;
	Ent = GetClientAimTarget(Client, false);
	...
	GetEdictClassname(Ent, name, sizeof(name));
	CheatCommand(Client, "ent_remove_all", name);
	return Plugin_Handled;
}

stock CheatCommand(client, String:command[], String:parametres[])
{
	if (client)
	{
		new flags = GetCommandFlags(command);
		SetCommandFlags(command, flags & ~FCVAR_CHEAT);
		FakeClientCommand(client, "%s %s", command, parametres);
