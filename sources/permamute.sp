...
enum PCommType
{
	PCommType_PMute = 0, PCommType_PUnMute, PCommType_PGag, PCommType_PUnGag, PCommType_PSilence, PCommType_PUnSilence, PCommType_NumTypes
}
...
public OnPluginStart()
{
	...
	g_cvars[CVAR_VERSION] = CreateConVar("sm_permamute_version", PERMAMUTE_VERSION, "PermaMute Version", FCVAR_PLUGIN|FCVAR_REPLICATED|FCVAR_NOTIFY);

	g_cookies[COOKIE_PMUTE] = RegClientCookie("permamute-mute", "PermaMute mute status", CookieAccess_Protected);

	g_cookies[COOKIE_PGAG] = RegClientCookie("permamute-gag", "PermaMute gag status", CookieAccess_Protected);
...
public AdminMenu_PMute(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
	...
	switch (action)
	{
		...
		{
			Format(buffer, maxlength, "Управление чатом и микрофоном");
		}
...
stock DisplayPMutePlayerMenu(client)
{
	...
	Format(title, sizeof(title), "Отключить игроку чат/микрофон:");
...
public MenuHandler_PMutePlayer(Handle:menu, MenuAction:action, param1, param2)
{
	...
	switch (action)
	{
		...
		case MenuAction_Select:
		{
			...
			if (!target)
			{
				PrintToChat(client, "\x04[Отключение чата/микрофона] Игрок более недоступен");
			}
			else if (!CanUserTarget(client, target))
			{
				PrintToChat(client, "\x04[Отключение чата/микрофона] Не могу выбрать цель");
			}
			else
			{
...
stock DisplayPMuteTypesMenu(client, target)
{
	...
	decl String:title[100];
	Format(title, sizeof(title), "Выберите:");
	SetMenuTitle(menu, title);
	SetMenuExitBackButton(menu, true);
	...
	if (!strcmp(cookie, "1"))
	{
		AddMenuItem(menu, "1", "Снять глушение игрока");
	}
	else
	{
		AddMenuItem(menu, "0", "Заглушить игрока");
		silenced = false;
	}
	...
	if (!strcmp(cookie, "1"))
	{
		AddMenuItem(menu, "3", "Включить игроку чат");
	}
	else
	{
		AddMenuItem(menu, "2", "Отключить игроку чат");
		silenced = false;
	}

	if (silenced)
	{
		AddMenuItem(menu, "5", "Включить игроку чат и микрофон");
	}
	else
	{
		AddMenuItem(menu, "4", "Отключить игроку чат и микрофон");
	}
...
public Menu_Status(client, CookieMenuAction:action, any:info, String:buffer[], maxlen)
{
	...
	{
		Format(buffer, maxlen, "Статус чата/микрофона");
	}
...
stock PerformPMute(client, target, PCommType:type)
{
	...
	switch (type)
	{
		case PCommType_PMute:
		{
			...
			PrintToChatAll("\x05Игроку \x04%N\x05 запрещен микрофон", target);
		}
		case PCommType_PUnMute:
		{
			...
			PrintToChatAll("\x05Игроку \x04%N\x05 разрешен микрофон", target);
		}
		case PCommType_PGag:
		{
			...
			PrintToChatAll("\x05Игроку \x04%N\x05 запрещен чат", target);
		}
		case PCommType_PUnGag:
		{
			...
			PrintToChatAll("\x05Игроку \x04%N\x05 разрешен чат", target);
		}
...
stock CreateMenuStatus(client)
{
	...
	Format(text, sizeof(text), "Статус чата/микрофона");
	SetMenuTitle(menu, text);

	GetClientCookie(client, g_cookies[COOKIE_PMUTE], cookie, sizeof(cookie));
	if (!strcmp(cookie, "1"))
	{
		AddMenuItem(menu, "permamute-mute", "Ваш микрофон запрещен", ITEMDRAW_DISABLED);
	}
	else
	{
		AddMenuItem(menu, "permamute-mute", "Ваш микрофон разрешен", ITEMDRAW_DISABLED);
	}

	GetClientCookie(client, g_cookies[COOKIE_PGAG], cookie, sizeof(cookie));
	if (!strcmp(cookie, "1"))
	{
		AddMenuItem(menu, "permamute-gag", "Вам запрещен чат", ITEMDRAW_DISABLED);
	}
	else
	{
		AddMenuItem(menu, "permamute-gag", "Вам разрешен чат", ITEMDRAW_DISABLED);
	}
...
stock TargetedAction(client, PCommType:type, const String:target_string[])
{
	...
	if ((target_count = ProcessTargetString(target_string, client, target_list,	MAXPLAYERS,	0, target_name,	sizeof(target_name), tn_is_ml)) <= 0)
	{
