...
public OnPluginStart()
{
	...
	hMaxInfected	= CreateConVar("l4d_multislots_max_infected", "5", "How many infected allowed?", CVAR_FLAGS, true, 4.0, true, 32.0);
...
public Action:JoinTeam(client, args)
{
	...
	if(IsClientInGame(client))
	{
		...
		{	
			if (DispatchKeyValue(client, "classname", "player") == true) PrintHintText(client, "You are allready joined the Survivor team");
			else if ((DispatchKeyValue(client, "classname", "info_survivor_position") == true) && !IsAlive(client)) PrintHintText(client, "Please wait to be revived or rescued");
		}
		else if (IsClientIdle(client)) PrintHintText(client, "You are now idle. Press mouse to play as survivor")
		else
		{			
			if (TotalFreeBots() == 0)
			{
...
public evtRoundStartAndItemPickup(Handle:event, const String:name[], bool:dontBroadcast)
{
	if (!gbFirstItemPickedUp)
	{
		...
		if (timer_SpecCheck == INVALID_HANDLE) timer_SpecCheck = CreateTimer(15.0, Timer_SpecCheck, _, TIMER_REPEAT);
...
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"))
	if (client)	if ((GetClientTeam(client) != TEAM_INFECTED) && (GetClientTeam(client) != TEAM_SURVIVORS) && !IsFakeClient(client) && !IsClientIdle(client)) CreateTimer(DELAY_CHANGETEAM_NEWPLAYER, Timer_AutoJoinTeam, client, TIMER_REPEAT);
}
public evtPlayerLeftStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(event, "userid"))
	if (client && IsClientConnected(client) && IsClientInGame(client) && GetClientTeam(client) == TEAM_SURVIVORS) gbPlayedAsSurvivorBefore[client] = true;
}
...
public evtPlayerTeam(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client)
	{
		...
		if (newteam == TEAM_INFECTED) giIdleTicks[client] = 0;
	}
}
...
{
	for (new i = 1; i <= MaxClients; i++) if (IsClientConnected(i) && IsClientInGame(i) && GetClientTeam(i) == TEAM_SURVIVORS && IsAlive(i))
	{
...
public Action:Timer_SpecCheck(Handle:timer)
{
	...
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsClientConnected(i) && IsClientInGame(i))
		{
			if ((GetClientTeam(i) == TEAM_SPECTATORS) && !IsFakeClient(i))
			{
				if (!IsClientIdle(i))
				{
					PrintToChat(i, "\x01%N, type \x03!join\x01 to join the Survivor Team", i);
				}
	...
	// for (new i = 1; i <= MaxClients; i++) if (IsClientConnected(i) && IsClientInGame(i) && (GetClientTeam(i) == TEAM_SURVIVORS) && !IsFakeClient(i) && !IsAlive(i)) PrintToChat(i, "\x01[\x04MultiSlots\x01] %N, please wait to be revived or rescued", i)
	return Plugin_Continue;
}
...
public Action:Timer_AutoJoinTeam(Handle:timer, any:client)
{
	...
	if (IsClientInGame(client))
	{
		if (GetClientTeam(client) == TEAM_SURVIVORS || IsClientIdle(client)) return Plugin_Stop;
		JoinTeam(client, 0);
	}
	return Plugin_Continue;
}
