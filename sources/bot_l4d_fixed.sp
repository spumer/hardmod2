...
{  
	l4d_bot_max = CreateConVar("l4d_bot_max", "16", "max bots");
...
//	new team =  GetEventInt(event, "team");
	...
	if (isnew && client && IsClientInGame(client) && !IsFakeClient(client))	CreateTimer(1.0, ClientInServer, client);
}
...
public Action:ClientInServer(Handle:timer, any:client)
{
	...
	{ 
		...
		{
			new bot = GetABot();
			if (bot) TakeOverBot(client, bot);
		}
		new bool:bstate = GetAliveState(client);
		if (bstate && !IsPlayerAlive(client)) Respawn(client);
		if (GetConVarInt(l4d_bot_teleport) == 1) CreateTimer(0.1, TeleMe, client);
	}
}

public Action:TeleMe(Handle:timer, any:client)
{
	if (IsClientInGame(client)) TeleportClientTo(client);
}

public Action:player_death(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	if (client && IsClientInGame(client) && GetClientTeam(client) == 2 && !IsFakeClient(client)) SetAliveState(client, false);
}

public Action:player_spawn(Handle:hEvent, const String:strName[], bool:DontBroadcast)
{
	new client = GetClientOfUserId(GetEventInt(hEvent, "userid"));
	if (client && IsClientInGame(client) && GetClientTeam(client) == 2 && !IsFakeClient(client)) SetAliveState(client, true);
}
...
public OnClientPutInServer(client)
{
	if (client && !IsFakeClient(client))
	{
		...
		if (bot < player && bot < GetConVarInt(l4d_bot_max)) CreateBot();
	}
}
...
{
	new count = 0;
	for (new i = 1; i < MaxClients; i++) if (IsClientInGame(i) && GetClientTeam(i) == 2) count++;
	return count;
}
...
{
	if (IsClientConnected(bot))	KickClient(bot, "fake player");
	return Plugin_Stop;
}
...
{
	new count = 0;
	for (new i = 1; i < MaxClients; i++) if (IsClientInGame(i) && GetClientTeam(i) != 3 && !IsFakeClient(i) && i != except) count++;
	return count;
}
...
stock Respawn(client)
{
	...
	if (hRoundRespawn == INVALID_HANDLE)
	{
		...
		if (hRoundRespawn == INVALID_HANDLE) PrintToChatAll("L4D_SM_Respawn: RoundRespawn Signature broken");
  	}
...
MakeBot(client=0)
{
	...
	if (bot != 0)
	{
		...
		SetEntityRenderColor(bot, 128, 0, 0, 255);
		CreateTimer(0.1, TimerKick, bot, TIMER_FLAG_NO_MAPCHANGE);
		if (client) PrintToChatAll("Create a bot for %N", client);
		if (client) PrintToServer("Create a bot for %N", client);
		decl Float:VecPosition[3];
		if (GetClientAimedLocationData(client, VecPosition, NULL_VECTOR, NULL_VECTOR) == -1) return 0;
...
{
	if (!client && !IsDedicatedServer()) return 1;
	return client;
}