...
public ReplyToCommandEx(any:client, String:Message[256])
{
	PrintToChat(client, "\x05[RZ]\x03 %s", Message);
}
...
public CheckPermissions(client)
{
	...
	{
		ReplyToCommandEx(client, "You don't have enough permissions to use this command.");
		return false;
	}
...
public Action:CmdDeleteAll (client, args)
{
	...
	if (DeleteAllCount == 0)
	{
		...
		ReplyToCommandEx(client, "Please execute this command again to confirm that you want to delete all the restricted zones for this map.");
		DeleteAllCount = 1;
	...
	if (!DeleteAllZones())
		ReplyToCommandEx(client, "There were no restricted zones stored for this map!");
	else
	ReplyToCommandEx(client, "All the restricted zones for this map has been deleted!");
...
public Action:CmdDeleteNear (client, args)
{
	...
	else
	{
		...
		{
			ReplyToCommandEx(client, "The radius must be between 10-1000. Command aborted.");
...
DeleteNearSpots(client, Float:radius)
{
	...
	if (deletedCount > 0)
	{
		...
		if (!StoreAllRestrictedZones()) 
		{
			...
			new String:S1[256];
			Format(S1, 255, "An error was found when deleting the restricted zones ( %d ).", deletedCount);
			ReplyToCommandEx(client, S1);
		}
		else
		{
			// We reply the command
			new String:S1[256];
			Format(S1, 255, "%i zones has been deleted.", deletedCount);
			ReplyToCommandEx(client, S1);
		}
	...
	{
		ReplyToCommandEx(client, "There were no restricted zones near the current location.");
	}
}
...
public Action:CmdStoreLocation (client, args)
{
	...
	{
		new String:S1[256];
		Format(S1, 255, "The maximum of %i restricted zones has been reached.", MAX_RESTRICTEDSPOTS);
		ReplyToCommandEx(client, S1);
	...
	new String:S1[256];
	Format(S1, 255, "Restricted zone noted. Radious set on %f. Use rz_storemoveto to indicate the 'move to' location and store the zone.", radius);
	ReplyToCommandEx(client, S1);	
...
public Action:CmdStoreMoveTo(client, args)
{
	...
	{
		ReplyToCommandEx(client, "You need to first select a restricted location with the rz_storeloc.");
	...
	{
		ReplyToCommandEx(client, "This location is too close to the restricted spot. Please move away from it.");
	...
	if (!StoreRestrictedZone())
		ReplyToCommandEx(client, "There was an error storing the restricted zone.");
	else
		ReplyToCommandEx(client, "Restricted zone was stored.");
