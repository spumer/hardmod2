...
public OnPluginStart()
{
	...
	powerups_plugin_on = CreateConVar("l4d_powerups_plugin_on", "1", "Is the Plugin active? (1 = ON  0 = OFF)", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);
	powerups_duration = CreateConVar("l4d_powerups_duration", "60", "How long should the duration of the boosts last?", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 1.0);
	powerups_use_on = CreateConVar("l4d_powerups_use_on", "1", "Should players use adrenaline (or pills) to get super reload, weapon firing, and melee swings? (1 = ON  0 = OFF)", FCVAR_PLUGIN|FCVAR_SPONLY, true, 0.0, true, 1.0);
//	pills_luck = CreateConVar("l4d_powerups_pills_luck", "1", "The luck factor for swallowing pills that will grant the boost(s). (1 = 1/1  2 = 1/2  3 = 1/3  4 = 1/4  etc.)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 1.0);
	
	g_h_reload_rate = CreateConVar("l4d_powerups_weaponreload_rate", "0.9",	"The interval incurred by reloading is multiplied by this value (clamped between 0.2 < 0.9)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.2, true, 1.0);
	HookConVarChange(g_h_reload_rate, Convar_Reload);
	g_fl_reload_rate = 0.5714;
		
	g_h_melee_rate = CreateConVar("l4d_powerups_weaponmelee_rate", "0.5", "The interval for swinging melee weapon (clamped between 0.3 < 0.9)",	FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.3, true, 0.9);
		
	g_hDT_rate = CreateConVar("l4d_powerups_weaponfiring_rate", "1.0" , "The interval between bullets fired is multiplied by this value. WARNING: a short enough interval will make some guns firing accuracy distorted (clamped between 0.2 < 0.9)", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.2, true, 1.0);
